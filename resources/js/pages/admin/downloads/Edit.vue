...
const form = useForm({
  ...
  brand: props.download.brand || '',
  category: props.download.category,
  is_featured: props.download.is_featured,
  is_public: props.download.is_public,
  sort_order: props.download.sort_order,
  meta_title: props.download.meta_title || '',
  meta_description: props.download.meta_description || '',
  tags: Array.isArray(props.download.tags) ? [...props.download.tags] : [],
...
// Reactive state
const activeTab = useStorage('downloads_edit_active_tab', 'basic');
const posterPreview = ref<string | null>(props.download.poster_url);
const downloadFileInfo = ref<{ name: string; size: number; type: string } | null>(
  props.download.download_url ? {
    name: `${props.download.title}.${props.download.file_extension}`,
    size: props.download.file_size || 0,
...
const replacingPoster = ref(false);
const replacingDownloadFile = ref(false);
...
const canSubmit = computed(() => {
  return form.title && !form.processing && hasChanges.value;
});
...
const completionPercentage = computed(() => {
  ...
  if (downloadFileInfo.value) completed++;
...
const handlePosterUpload = (event: Event) => {
  ...
  if (file) {
    ...
    showNotification('New poster image selected!', 'success');
  }
};
...
const handleDownloadFileUpload = (event: Event) => {
  ...
  if (file) {
    ...
    replacingDownloadFile.value = true;
    ...
    showNotification('New download file selected!', 'success');
  }
};
  ...
  if (fileInfoEl) {
    gsap.to(fileInfoEl, {
      ...
      onComplete: () => {
        ...
        replacingDownloadFile.value = false;
      }
    });
  } else {
    form.download_file = null;
...
const submit = () => {
  ...
  form.post(route('admin.downloads.update', props.download.uuid), {
    onSuccess: () => {
    ...
    onError: () => {
      isUploading.value = false;
      uploadProgress.value = 0;
      showNotification('Failed to update download. Please try again.', 'error');
    }
  });
};
...
const resetForm = () => {
  form.reset();
  posterPreview.value = props.download.poster_url;
  downloadFileInfo.value = props.download.download_url ? {
    name: `${props.download.title}.${props.download.file_extension}`,
    size: props.download.file_size || 0,
    type: 'application/octet-stream'
  } : null;
        ...
        <div class="flex h-16 items-center justify-between">
          ...
          <div class="flex items-center gap-4">
            ...
            <div class="flex items-center gap-3">
              ...
              <div class="hidden sm:block">
                <h1 class="text-lg font-semibold">Edit Download</h1>
                <p class="text-sm text-muted-foreground">{{ download.title }}</p>
              </div>
            </div>
          </div>
          ...
          <div class="flex items-center gap-3">
            ...
            <div class="hidden lg:flex items-center gap-4 text-sm text-muted-foreground">
              ...
              <div class="flex items-center gap-1">
                <Eye class="h-4 w-4" />
                <span>{{ download.is_public ? 'Public' : 'Private' }}</span>
              </div>
            </div>
            ...
            <div class="hidden md:flex items-center gap-3">
              ...
              <Badge :variant="hasChanges ? 'destructive' : 'default'" class="gap-1">
                <AlertCircle v-if="hasChanges" class="h-3 w-3" />
...
<!--            </Button>-->
    ...
    <div v-if="isUploading" class="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <Card class="w-full max-w-md upload-progress">
        <CardContent class="p-6 text-center space-y-4">
          ...
          <div>
            <h3 class="font-semibold">Updating Download</h3>
            <p class="text-sm text-muted-foreground">Processing your changes...</p>
          </div>
          ...
          <div v-if="showSuccessAnimation" class="text-green-600">
            ...
            <p class="text-sm mt-1">Updated!</p>
          </div>
        </CardContent>
      </Card>
    </div>
    ...
    <div ref="contentRef" class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="md:hidden mb-6">
        ...
        <Progress v-model="completionPercentage" class="h-2" />
      </div>

      <!-- Unsaved Changes Alert -->
      <Alert v-if="hasChanges" class="mb-6 border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-950">
        ...
        <TabsContent value="basic" class="space-y-6">
          ...
          <Card ref="el => cardsRef[0] = el as HTMLElement">
            <CardHeader>
              ...
              <CardDescription>
                Update the essential details for your download
              </CardDescription>
            </CardHeader>
          ...
          <Card ref="el => cardsRef[2] = el as HTMLElement">
            <CardHeader>
              <CardTitle class="flex items-center gap-2">
                <Image class="h-5 w-5" />
                Poster Image
                <Badge v-if="replacingPoster" variant="secondary" class="ml-2">
              ...
              <div v-else class="poster-preview relative">
                ...
                <div class="absolute right-2 top-2 flex gap-2">
                  <Button
                    ...
                    @click="() => document.querySelector('input[type=file][accept*=image]')?.click()"
                  >
                    <RefreshCw class="h-4 w-4" />
                  </Button>
                  <Button
          ...
          <Card ref="el => cardsRef[3] = el as HTMLElement">
            <CardHeader>
              <CardTitle class="flex items-center gap-2">
                <FileText class="h-5 w-5" />
                Download File *
                <Badge v-if="replacingDownloadFile" variant="secondary" class="ml-2">
                  <RefreshCw class="h-3 w-3 mr-1" />
                  Replacing
                </Badge>
              </CardTitle>
              ...
              <div v-else class="file-info flex items-center justify-between rounded-lg border p-4">
                ...
                <div class="flex gap-2">
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    @click="() => document.querySelector('input[type=file]:not([accept])')?.click()"
                  >
                    <RefreshCw class="h-4 w-4" />
                  </Button>
                  <Button
